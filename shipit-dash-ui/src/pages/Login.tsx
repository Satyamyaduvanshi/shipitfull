  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        setAuthSession(registerResponse.user_id, username.trim());
        toast({
        ...
        setAuthSession(loginResponse.user_id, loginResponse.username);
        toast({
          title: "Welcome back",
          description: `Signed in as ${loginResponse.username}.`,
        });
      }
      ...
      const description =
        error instanceof ApiError
          ? error.message
          : "Unable to connect to backend. Verify API URL and server status.";
      ...
      >
        <Card className="p-8 bg-card/80 backdrop-blur-xl border-border shadow-2xl">
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ...
            <Button
              variant="link"
              className="px-0 text-foreground font-medium"
        ...
        <p className="text-center text-xs text-muted-foreground mt-6">
          ...
          <Button variant="link" className="px-0 text-xs text-muted-foreground underline">
            Terms of Service
          </Button>{" "}
          and{" "}
          <Button variant="link" className="px-0 text-xs text-muted-foreground underline">
